...
from .config_reader import ENVIRONMENT_CONFIG_ATTRIBUTES, ConfigReader
...
def validate_template(ctx, environment, stack):
    ...
    """
    stack = get_stack(ctx.obj["sceptre_dir"], environment, stack)
    result = stack.template.validate()
...
def describe_env_resources(ctx, environment):
    ...
    """
    env = get_env(ctx.obj["sceptre_dir"], environment)
...
def get_env(sceptre_dir, environment):
    """
...
def get_stack(sceptre_dir, environment, stack):
    """
